...
from utils.sd_samplers_common import SamplerData, setup_img2img_steps
...
samplers_data_k_diffusion = [
    SamplerData(label, lambda model, funcname=funcname: KDiffusionSampler(
...
class CFGDenoiser(torch.nn.Module):
        ...
        for i, conds in enumerate(conds_list):
            for cond_index, weight in conds:
                denoised[i] += (x_out[cond_index] -
                                denoised_uncond[i]) * (weight * cond_scale)
    ...
    def forward(self, x, sigma, uncond, cond, cond_scale, image_cond):
        ...
        # is_edit_model = sd_model.cond_stage_key == "edit" and self.image_cfg_scale is not None and self.image_cfg_scale != 1.0
        is_edit_model = False
        ...
        if not is_edit_model:
            denoised = self.combine_denoised(
                x_out, conds_list, uncond, cond_scale)
        else:
...
class KDiffusionSampler:
    ...
    def initialize(self, p):
        ...
        self.model_wrap_cfg.image_cfg_scale = getattr(
            p, 'image_cfg_scale', None)
        self.eta = p.eta
    ...
    def get_sigmas(self, p, steps):
        ...
        if p.sampler_noise_scheduler_override:
            ...
            sigmas = k_diffusion.sampling.get_sigmas_karras(
                n=steps, sigma_min=sigma_min, sigma_max=sigma_max, device=device)
        else:
